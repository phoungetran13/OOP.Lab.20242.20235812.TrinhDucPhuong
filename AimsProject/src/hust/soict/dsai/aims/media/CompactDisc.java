...
public class CompactDisc extends Disc implements Playable {
    ...
    @Override
    public void play() throws PlayerException { // THAY ĐỔI
        if (this.getLength() <= 0) {
        ...
        if (!tracks.isEmpty()) {
             ...
             // Lặp qua các track và xử lý ngoại lệ cho từng track
             for (Track track : tracks) {
                 try {
                     track.play();
                 } catch (PlayerException e) {
                     // In lỗi của track ra nhưng vẫn tiếp tục phát các track khác
                     // Hoặc có thể ném lại lỗi để dừng cả CD
                     System.err.println(e.getMessage());
                     // throw e; // Nếu muốn dừng cả CD khi một track lỗi
                 }
             }
             System.out.println("--------------");
        } else {
